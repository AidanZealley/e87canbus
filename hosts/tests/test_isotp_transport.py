from __future__ import annotations

import isotp.protocol
import pytest
from e87canbus.config import CanNetwork
from e87canbus.protocol.can import CanFrame
from e87canbus.protocol.generated import (
    CAN_ID_SERVOTRONIC_TRANSPORT_COORDINATOR_TO_DEVICE,
    CAN_ID_SERVOTRONIC_TRANSPORT_DEVICE_TO_COORDINATOR,
    SERVOTRONIC_TRANSPORT_MAXIMUM_PAYLOAD_LENGTH,
)
from e87canbus.runners.simulation.bus import InMemoryCanTopology
from e87canbus.transport.isotp import IsoTpEndpoint


def _endpoints() -> tuple[IsoTpEndpoint, IsoTpEndpoint]:
    topology = InMemoryCanTopology(clock=lambda: 0.0)
    coordinator_bus = topology.create_bus(CanNetwork.KCAN, "coordinator")
    device_bus = topology.create_bus(CanNetwork.KCAN, "servotronic")
    coordinator = IsoTpEndpoint(
        tx_id=CAN_ID_SERVOTRONIC_TRANSPORT_COORDINATOR_TO_DEVICE,
        rx_id=CAN_ID_SERVOTRONIC_TRANSPORT_DEVICE_TO_COORDINATOR,
        send_frame=coordinator_bus.send,
    )
    device = IsoTpEndpoint(
        tx_id=CAN_ID_SERVOTRONIC_TRANSPORT_DEVICE_TO_COORDINATOR,
        rx_id=CAN_ID_SERVOTRONIC_TRANSPORT_COORDINATOR_TO_DEVICE,
        send_frame=device_bus.send,
    )

    def pump() -> None:
        for _ in range(64):
            coordinator.poll()
            device.poll()
            progressed = False
            for bus, endpoint in ((coordinator_bus, coordinator), (device_bus, device)):
                while (frame := bus.receive(0)) is not None:
                    endpoint.on_frame(frame)
                    progressed = True
            if not progressed and not coordinator.transmitting and not device.transmitting:
                return
        raise AssertionError("ISO-TP transfer did not settle")

    coordinator.pump = pump  # type: ignore[attr-defined]
    return coordinator, device


def test_iso_tp_exchanges_multi_frame_opaque_payloads_over_simulated_can() -> None:
    coordinator, device = _endpoints()
    coordinator.send(bytes(range(48)))
    coordinator.pump()  # type: ignore[attr-defined]
    assert device.receive_payload() == bytes(range(48))

    device.send(bytes(range(48)))
    coordinator.pump()  # type: ignore[attr-defined]
    assert coordinator.receive_payload() == bytes(range(48))


def test_iso_tp_latest_value_overwrites_pending_before_dispatch() -> None:
    coordinator, device = _endpoints()
    coordinator.send(b"first")
    coordinator.send(b"second")  # overwrites before any poll; only second is transmitted
    coordinator.pump()  # type: ignore[attr-defined]
    assert device.receive_payload() == b"second"
    assert device.receive_payload() is None


def test_iso_tp_sends_one_latest_value_program_in_command_order() -> None:
    coordinator, device = _endpoints()
    coordinator.send_many((b"replace", b"button-1", b"button-15"))
    coordinator.pump()  # type: ignore[attr-defined]

    assert [device.receive_payload(), device.receive_payload(), device.receive_payload()] == [
        b"replace",
        b"button-1",
        b"button-15",
    ]
    assert device.receive_payload() is None


def test_iso_tp_new_program_replaces_all_commands_not_yet_dispatched() -> None:
    coordinator, device = _endpoints()
    coordinator.send_many((b"old-replace", b"old-override"))
    coordinator.send_many((b"new-replace", b"new-override"))
    coordinator.pump()  # type: ignore[attr-defined]

    assert device.receive_payload() == b"new-replace"
    assert device.receive_payload() == b"new-override"
    assert device.receive_payload() is None


def test_iso_tp_can_space_commands_without_delaying_the_first() -> None:
    now = 10.0
    sent: list[CanFrame] = []
    endpoint = IsoTpEndpoint(
        tx_id=0x708,
        rx_id=0x709,
        send_frame=sent.append,
        minimum_payload_interval_s=0.25,
        clock=lambda: now,
    )
    endpoint.send_many((b"first", b"second"))

    endpoint.poll()
    assert len(sent) == 1
    endpoint.poll()
    assert len(sent) == 1

    now += 0.25
    endpoint.poll()
    assert len(sent) == 2


def test_iso_tp_rejects_oversize_payload_and_ignores_other_frames() -> None:
    coordinator, _ = _endpoints()
    with pytest.raises(ValueError, match="64"):
        coordinator.send(b"x" * (SERVOTRONIC_TRANSPORT_MAXIMUM_PAYLOAD_LENGTH + 1))
    assert not coordinator.on_frame(CanFrame(0x123, b"\x00"))


def test_iso_tp_discards_bad_sequence_and_timed_out_partial_transfers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    now_ns = 0
    monkeypatch.setattr(isotp.protocol.time, "perf_counter_ns", lambda: now_ns)
    receiver = IsoTpEndpoint(tx_id=0x709, rx_id=0x708, send_frame=lambda frame: None)
    assert receiver.on_frame(CanFrame(0x708, b"\x10\x08abcdef"))
    receiver.poll()
    assert receiver.on_frame(CanFrame(0x708, b"\x22gh"))
    receiver.poll()
    assert receiver.receive_payload() is None

    assert receiver.on_frame(CanFrame(0x708, b"\x10\x08abcdef"))
    receiver.poll()
    now_ns = 2_000_000_000
    receiver.poll()
    assert receiver.receive_payload() is None
