...
import { CanTraceTable } from "@/components/CanTraceTable"
import { FrameDetail } from "@/components/FrameDetail"
import { LedGrid } from "@/components/LedGrid"
import { NeoTrellisPanel } from "@/components/NeoTrellisPanel"
import { SimulatorToolbar } from "@/components/SimulatorToolbar"
import { connectSimulatorSocket, getSnapshot, pressButton, releaseButton, resetSimulator, stepSimulator, toggleButton } from "@/lib/api"
import type { CanTraceEntry, SimulatorEvent, SimulatorSnapshot } from "@/lib/simulatorTypes"
...
export default function App() {
  ...
  const applySnapshot = useCallback((next: SimulatorSnapshot) => {
    ...
    setSelectedFrame((current) => {
      if (!current) return next.trace.at(-1) ?? null
      return next.trace.find((entry) => entry.monotonic_s === current.monotonic_s) ?? next.trace.at(-1) ?? null
    })
  }, [])
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="grid gap-4 p-4 lg:grid-cols-[320px_320px_minmax(0,1fr)]">
        <NeoTrellisPanel
