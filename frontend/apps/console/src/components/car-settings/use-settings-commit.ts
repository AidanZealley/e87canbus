...
const settingsMutationKey = ["application-settings", "update"] as const
...
export const useSettingsCommit = (settings: ApplicationSettingsResponse) => {
  ...
  const commit = async (patch: Partial<ApplicationSettingsValues>) => {
    if (
      !synchronized ||
      queryClient.isMutating({ mutationKey: settingsMutationKey, exact: true })
    ) {
      return
    }
  ...
  return { commit, canCommit: synchronized && !update.isPending }
}
