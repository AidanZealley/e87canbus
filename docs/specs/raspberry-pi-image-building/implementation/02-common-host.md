...
- First-boot boundary: `/usr/share/e87canbus/provisioning-interface` contains version `1`, and the
  ...
  provisioning feature owns the consumer that validates and installs the application and
  provisioning data before removing the marker. No consumer or provisioning unit is present in
  these images. This freezes no boot-partition path, payload schema, credential format or secret
  transport.
...
- Known limitations or external checks: The accepted artifacts are validated host-image
  prototypes, not provisionable images. Each artifact contains the hostname generated by the
  upstream image build, so every card flashed from that artifact clones the hostname. The later
  provisioning feature must decide the bundle, validation and secret lifecycle, add the image-side
  consumer that creates unique host identity, return that source change through the image builder,
  and rebuild and rerun the relevant hardware checks before provisioning ships. Role layers still
  need to install and gate their units. Docker image construction and Pi behavior remain for the
  final role-image checkpoint.
- Specification drift: Approved by Aidan on 2026-09-09 after the whole-feature review. The original
  image-building specification required this workstream to install the one-time provisioning
  service. The accepted implementation stops at a versioned interface marker, protected
  unprovisioned state and fail-closed service gates. It also retains the upstream build-time
  hostname, which clones across cards flashed from one artifact. The product and provisioning
  specifications now assign the consumer, creation of unique host identity and the required image
  rebuild to the later provisioning feature. The rebuilt provisioning-compatible images must
  replace these prototypes.
