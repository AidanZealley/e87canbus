...
The coordinator image also contains nginx, dnsmasq and nftables. Provisioning supplies the
WPA2-Personal RSN/CCMP access-point profile with required management-frame protection and the TLS
...
gateway. Avahi advertises `e87.local` only through IPv4 mDNS on `wlan0`; the certificate includes
that name. The firewall drops forwarding and permits Wi-Fi ingress only for DHCP, mDNS, HTTPS, SSH
and ICMP. The controller service has no privileged NetworkManager helper or sudo rule. NetworkManager
autoconnect owns access-point activation, and the coordinator panel cannot change it.

The console image contains Cage, Chromium and NSS tools. Provisioning supplies the same Wi-Fi
security policy with static address `10.42.0.2/24`, imports its client identity into the
`e87-kiosk` Chromium profile and installs automatic certificate selection only for
`https://10.42.0.1`. Maintenance laptops can use `https://e87.local`. The isolated AP intentionally
has no internet route, so a maintenance laptop may show a no-internet indicator. Existing devices
remain available by IP and need a fresh generated bundle and reprovisioning to gain the friendly
name in their server certificate.
...
checks cover the provisioned `kcan` mode, listen-only for `car` and acknowledgement-enabled for
`bench`, plus DRM and touchscreen input. Network checks cover the `10.42.0.1/24`
